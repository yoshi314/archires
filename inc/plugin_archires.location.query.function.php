...
function plugin_archires_display_Query_Location ($ID,$PluginArchiresQueryLocation,$PluginArchiresView,$for){
	...
	if ($PluginArchiresView->fields["networking"]!=0)
	 ...
	 foreach ($types as $key => $val){
      ...
      $typefield = "phonestypes_id";
		...
		if ($PluginArchiresQueryLocation->fields["vlans_id"] > "0")
		...
		if ($PluginArchiresQueryLocation->fields["vlans_id"] > "0")
			$query .= " AND `nv`.`networkports_id` = `np`.`id` 
					AND `vlans_id` = '".$PluginArchiresQueryLocation->fields["vlans_id"]."'";
					
		if ($PluginArchiresQueryLocation->fields["networks_id"] > "0" && $val != PHONE_TYPE && $val != PERIPHERAL_TYPE)
			$query .= " AND `$LINK_ID_TABLE[$val]`.`networks_id` = '".$PluginArchiresQueryLocation->fields["networks_id"]."'";
		if ($PluginArchiresQueryLocation->fields["states_id"] > "0")
			$query .= " AND `$LINK_ID_TABLE[$val]`.`states_id` = '".$PluginArchiresQueryLocation->fields["states_id"]."'";
		if ($PluginArchiresQueryLocation->fields["groups_id"] > "0")
			$query .= " AND `$LINK_ID_TABLE[$val]`.`groups_id` = '".$PluginArchiresQueryLocation->fields["groups_id"]."'";
		if ($PluginArchiresQueryLocation->fields["locations_id"]!="-1"){
			$query .= " AND `lc`.`id` = `$LINK_ID_TABLE[$val]`.`locations_id` 
						AND `lc`.`id` IN ('".$PluginArchiresQueryLocation->fields["locations_id"]."'";
			if ($PluginArchiresQueryLocation->fields["child"]!='0')
				$query .= plugin_archires_findChilds($DB, $PluginArchiresQueryLocation->fields["locations_id"]);
			$query .= ") ";
		 }else{
		  $query .= " AND `lc`.`id` = `$LINK_ID_TABLE[$val]`.`locations_id` 
					AND `lc`.`id` IN (0";
			$query .= plugin_archires_findLevels($DB, $PluginArchiresQueryLocation->fields["locations_id"]);
			if ($PluginArchiresQueryLocation->fields["child"]!='0')
				$query .= plugin_archires_findChilds($DB, $PluginArchiresQueryLocation->fields["locations_id"]);
			$query .= ") ";
		}
	  //types
	  $query .= plugin_archires_Query_Type_check(PLUGIN_ARCHIRES_LOCATIONS_QUERY,$ID,$val);
