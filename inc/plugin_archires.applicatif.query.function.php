...
function plugin_archires_display_Query_Applicatifs ($ID,$PluginArchiresQueryApplicatifs,$PluginArchiresConfig,$for){
	...
	if ($PluginArchiresConfig->fields["networking"]!=0)
	 ...
	 foreach ($types as $key => $val){
		...
		if ($PluginArchiresQueryApplicatifs->fields["FK_vlan"] > "0")
			$query .= ", `glpi_networking_vlan` nv";

		$query .= ", `glpi_plugin_applicatifs_device` app";
		$query .= " WHERE `np`.`device_type` = " . $val . " 
					AND `np`.`on_device` = `$LINK_ID_TABLE[$val]`.`ID` 
					AND `app`.`FK_device` = `$LINK_ID_TABLE[$val]`.`ID` ";
		...
		if ($PluginArchiresQueryApplicatifs->fields["FK_vlan"] > "0")
			$query .= " AND `nv`.`FK_port` = `np`.`ID` 
						AND `FK_vlan` = '".$PluginArchiresQueryApplicatifs->fields["FK_vlan"]."'";
		if ($PluginArchiresQueryApplicatifs->fields["network"] > "0")
			$query .= " AND `$LINK_ID_TABLE[$val]`.`network` = '".$PluginArchiresQueryApplicatifs->fields["network"]."'";
		if ($PluginArchiresQueryApplicatifs->fields["state"] > "0")
			$query .= " AND `$LINK_ID_TABLE[$val]`.`state` = '".$PluginArchiresQueryApplicatifs->fields["state"]."'";
		if ($PluginArchiresQueryApplicatifs->fields["FK_group"] > "0")
			$query .= " AND `$LINK_ID_TABLE[$val]`.`FK_groups` = '".$PluginArchiresQueryApplicatifs->fields["FK_group"]."'";
		
		$query .= " AND `app`.`FK_applicatif` = '" . $PluginArchiresQueryApplicatifs->fields["applicatifs"] . "' 
					AND `app`.`device_type` = " . $val . " ";
		
		$query .= plugin_archires_Query_Type_check(PLUGIN_ARCHIRES_APPLICATIFS_QUERY,$ID,$val);
